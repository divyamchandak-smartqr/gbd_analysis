...
#
# Output:
#   FOUR FINAL CSV FILES — one for each measure.
#
...
# One output CSV for each measure.
OUTPUT_FILES = {
    "DALYs": OUTPUT_DIR / "DALYs.csv",
    "Deaths": OUTPUT_DIR / "Deaths.csv",
    "YLDs": OUTPUT_DIR / "YLDs.csv",
    "YLLs": OUTPUT_DIR / "YLLs.csv",
}
...
def main() -> None:
    ...
    # 10. SAVE FOUR SEPARATE CSV FILES.
    #
    # The analysis logic and all source information remain unchanged.
    # Each CSV keeps:
    #   Cause ID, Cause Name, State, Gender
    #   + that measure's year-wise burden and YoY columns.
    #
    # Missing measure data remains BLANK exactly as before.
    base_columns = [
        "Cause ID",
        "Cause Name",
        "State",
    ...
    for measure in MEASURES:
        ...
        output_columns = base_columns + measure_columns
        measure_final = final[output_columns].copy()

        measure_final.to_csv(
            OUTPUT_FILES[measure],
            index=False,
        )
    ...
    for measure in MEASURES:
        print(f"{measure:<8}: {OUTPUT_FILES[measure].resolve()}")
